...
async function retryUpload(uploadFunction, retries = 10, retryDelay = 5000) {
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            console.error(`    > Attempt to upload token metadata to IPFS ${attempt} failed:`, error.message);

            if (attempt < retries) {
                console.log(`    > Retrying in ${retryDelay / 1000} seconds...`);
                await new Promise((resolve) => setTimeout(resolve, retryDelay));
            } else {
                console.error("    > All attempts failed. Please check your network connection or try again later.");
                throw error;
            }
        }
    }
